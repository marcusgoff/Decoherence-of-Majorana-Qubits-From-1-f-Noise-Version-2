...
function overlap = calc_overlap_cov_mats(A,B,N) 
    ...
    % 
    % if log(det(A + B)) > 800
    %     warning('Large log determinant > 800 in overlap formula');
    % end

    % - - - - - - - - - - %
    % Alternate code using chat gpt
    M = A + B;
    [L,U,P] = lu(M);
    
    logabsdet = sum(log(abs(diag(U))));
    logQ = -N*log(2) + 0.5*logabsdet;
    
    % Optional sign check
    sign_det = det(P)*prod(sign(diag(U)));
    % - - - - - - - - - - %
